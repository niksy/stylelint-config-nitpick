...
describe('Default config', function () {
	...
	it('should return proper validation errors for linted code', async function () {
		try {
			const { results } = await runStylelint('./fixtures/default.config.css', {
				extends: fileURLToPath(new URL('../index.js', import.meta.url))
			});
			const errors = mapErrors(results[0].warnings);
			assert.notEqual(errors.indexOf('declaration-property-value-disallowed-list'), -1);
...
describe('SCSS config', function () {
	...
	it('should return proper validation errors for linted code', async function () {
		try {
			const { results } = await runStylelint('./fixtures/scss.config.scss', {
				extends: fileURLToPath(new URL('../scss.js', import.meta.url))
			});
			const errors = mapErrors(results[0].warnings);
			assert.notEqual(errors.indexOf('scss/at-function-pattern'), -1);
			assert.notEqual(errors.indexOf('scss/at-import-partial-extension-disallowed-list'), -1);
